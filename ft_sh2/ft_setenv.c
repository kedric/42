...
/*   Updated: 2014/01/24 19:16:24 by jmancero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_environ(t_env *env)
{
	...
	while (env->environ[i[0]])
	{
		...
		else if (!ft_strncmp(env->environ[i[0]], "PWD=", 4))
			env->pwd = ft_strdup(env->environ[i[0]] + 4);
...
void	ft_unsetenv(char **av, t_env *env)
{
	...
	while (env->environ[j])
	{
		if (ft_strncmp(env->environ[i], av[1], ft_strlen(av[1])))
		{
			tmp[i] = env->environ[j];
			i++;
		}
